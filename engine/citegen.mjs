...
function audit(argv, opts = {}) {
  ...
  if (!opts.files && !targets.length)
    console.log(`  NOT COLLECTED by this default run: engine/qjs beyond quickjs.c/.h and engine/lexbor (upstream, not this tree's to answer for); ` +
      `generated bytes under out/ and .work/; and engine/*.mjs, the gates themselves — pointing this audit at seven of them returns 12 MISATTRIBUTED, ` +
      `of which 4 are this file's own WORKED EXAMPLES of wrong numbers (a comment that says "§7.4.9 IteratorClose was written 58 times" cites §7.4.9 in ` +
      `order to retire it). Until a citation the prose marks as RETIRED can be told from one the prose asserts, auditing the gates would plant permanent ` +
      `false reds in the auditor — so they are named here rather than counted as clean. Audit them anyway with: node engine/citegen.mjs engine/<gate>.mjs`);

 ...
 *
 * THREE MARKUP FAMILIES, ONE INDEX SHAPE. The standards this tree cites are generated by three different
 * pipelines and each needs its own reader, but all three produce the same {sections, dfns, uses} index, so the
 * audit knows nothing about any of them. Adding a standard is a row in SPECS plus one curl.
 *   — whatwg-multipage (HTML): §-numbered <h2>-<h6> across ~57 pages, terms as <dfn>.
 *   — bikeshed (DOM, URL, Fetch): one page, `data-level` on the heading, terms as <dfn>.

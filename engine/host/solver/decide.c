...
int solver_decide(JSContext *ctx, JSValueConst cond) {
    ...
    /* NEW decision at the frontier. Domain-based feasibility refinement comes with the constraint component;
       for now both arms are open -> FORK the FALSE sibling (a copy of the path so far + the FALSE arm), and this
       flow takes TRUE. The sibling is a flow_add: re-run g_cur_fn replaying (path ++ FALSE). No OP_if rewind. */

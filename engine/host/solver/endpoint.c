...
/* THE ASK, WHICH THE FOUR NUMBERS BESIDE THE SURFACE STRUCTURALLY CANNOT CARRY. endpoint_surface_census walks
   ...
   so a count taken at its entry is the number of times this engine COMPOSED a request and offered its address
   to this surface, which is a question about this engine's execution and not about its surface.
   THAT CLAUSE READ `the number of times a network call site was REACHED` AND THE INFERENCE IS INVALID — see
   endpoint.h's retirement of it, which names the five Fetch §5.4 stages standing in front of this door and the
   residual for the count that would make the stronger sentence true. It is rewritten rather than deleted
   because the premise above it is correct and a reader who re-derives the conclusion from it will re-add it.
   ...
   that take opposite work: no arm ever composed a request at all, or arms composed one and every address they
   built was already on the surface. The second is invisible to every row the census has,
   ...
   the second case and zero in the first, which is the separation it is offered as — and it is a CEILING on
   that reading rather than the reading itself, because it is nonzero for a post-program ask that MINTED and
   for one that merged into a record post-program code had itself minted. `merged_pre_program` below is the
   reading stated exactly.
   SIX ROWS, TWO PARTITIONS AND A CUT, OVER ONE EVENT. By OUTCOME, `suppressed + merged + minted == asks`,
   asserted where all four are in one hand; by PROGRAM STATE, `preProgram <= asks`, a containment rather than a
   second partition for `endpoint_surface_census`'s reason exactly — the complement is what a reader computes
   and a stored complement is a second raise free to drift from the first. The sixth is a CUT INSIDE the
   merged arm and is summed with nothing; its own two containments are asserted at the accessor.
...
/* AND THE ONE CUT OF THE MERGED ARM THAT ANSWERS THE RAZOR'S OWN SENTENCE, WHICH `asks - preProgram` STATES
   AND DOES NOT MEASURE. endpoint.h names two readings of a zero `emitted - preProgram` and offers that
   subtraction as the separation between them; it is nonzero in the arm it names (an ask made after a program
   started, reaching an address the surface already held) and ALSO in two arms it does not — a post-program ask
   that MINTED, and a post-program ask that merged into a record post-program code had itself minted. So the
   subtraction bounds the "reached and already known" reading from above and never states it. This is that
   reading: the ask was made with a program running AND the record it reached was minted before any program
   had run, which is running code composing an address the MARKUP had already named. Nothing else in either
   census can be that, because a merge moves no figure on the surface at all.
   IT IS A CUT AND NOT A FOURTH ARM. The three arms partition the door's exits; this selects inside one of
   them on a fact about the RECORD, so it is contained in `merged` and contained in `asks - preProgram`, and
   both containments are asserted in endpoint_ask_census where every term is in one hand. A stored complement
   would be a second raise free to drift from the first, which is the reason `preProgram` has none either.
   ITS ZERO IS FOUR STATES AND THE OTHER ROWS SEPARATE THEM, which is what makes it worth publishing rather
   than a fifth number nobody can read: no post-program ask was made at all (`asks == preProgram`); they were
   made and SUPPRESSED; they were made and MINTED, which `emitted - preProgram` then shows unless the reply
...
/* THE HOST EDGES' OWN ENTRIES — endpoint.h states both contracts in full. These live HERE and not in
   core/fetch/fetch.c and core/xhr/xml_http_request.c for two reasons, and the second is the one that could
   ...
   unreachable.
   AND ONE OF THE IDENTITIES SPANS THE TWO. `offered <= asks` is the relation that makes an edge's share of
   this surface's ask population readable, and it is checkable only where both terms are in one hand — which
   is here and nowhere else. The alternative was each edge holding its own counters and this file reading them
   back, which is the same number in two places with a call between them.
   THE LABELS ARE EACH MACHINE'S AND ARE HANDED OVER RATHER THAN COPIED. The `steps` field below is
   core/fetch's own `js_fetch_steps[]` or core/xhr's own `SEND_STEPS` — static tables of string literals, so
   they outlive every document and this file may key on them — and `stage_n` is derived from the NULL
   terminator. A stage added to either X-list adds a row here with no edit at all; a second list spelled here
   would be the drifting copy §AN-AUDITOR-DERIVES-THE-RULE forbids, and it would drift silently, because a
   missing row reads as a stage nothing died at. */
/* ONE STRUCT AND TWO INSTANCES, NOT TWO SETS OF STATICS. There are two host edges with this shape — core/
   fetch's request construction and core/xhr's `send()` — and the arithmetic over them is IDENTICAL: a
   partition over one teardown and a containment against the ask that preceded it. Spelling it twice would be
   two implementations of one mechanism with the identities in two places, which is the second copy
   §AN-AUDITOR-DERIVES-THE-RULE forbids and which drifts in the direction nobody sees, since a census that
   asserts less still publishes numbers.
   WHAT IS PER-EDGE AND STAYS PER-EDGE IS THE ROW NAMES. `json_buf_key` takes a STRING LITERAL and the
   compiler enforces it (core/json_buf.h states why), so a shared emitter could not compose `epFetch…` or
   `epXhr…` at all — and that is the property that makes every field name this seam publishes readable off
   this source. So the STATE and the ARITHMETIC are shared and the KEYS are written out per edge.
   `reached` IS THE FIELD EACH EDGE NAMES FOR ITSELF. For core/fetch it is the OFFER — the construction got to
   §5.6 step 12 and handed this surface an address. For core/xhr it is the PLACEMENT — the construction got to
   §3.5.6 step 12/13 and handed the request to the lifecycle machine, which is one machine away from the door
   and cannot be a fact about the send state that is already gone. Both are "the construction completed", the
   partition is over whichever it is, and each edge's rows spell which in their own names.
   EVERY COUNT IS A LIFETIME COUNT AND NONE IS A GAUGE — they may be differenced and accumulated, they cannot
   decrease, and a sample below its predecessor is this file and not the run. A REPORT AND NEVER A BOUND. */
typedef struct {
    const char *const *steps;   /* the edge's own stage labels, borrowed from its X-list — never a copy */
    ...
    long  freed_reached;        /* OUTCOME: …of those, the ones that had completed */
} EndpointEdge;

static EndpointEdge g_fetch_edge;
static EndpointEdge g_xhr_edge;
/* AND THE ONE ROW THAT IS NOT A FACT ABOUT ANY SEND STATE. core/xhr records from its LIFECYCLE machine, which
   the send state may already have been torn down before, so the edge's offer to this surface cannot live in
   the struct above beside a partition it does not belong to. It is the row that carries the `offered <= asks`
   containment for that edge, and endpoint.h states why no relation between it and `reached` may be asserted. */
static long  g_xhr_offered;
...
/* …AND THE ARRAY GIVEN BACK AND THE TABLE LET GO, in that order and both of them, because the next session's
   edge declares itself again at its own init: keeping the array would be a session's allocation outliving the
   session, and keeping the pointer while freeing the array would leave the next declare taking its early
   return and every teardown indexing a freed block. */
static void edge_release(EndpointEdge *e) {
    edge_reset(e);
    free(e->died);
    e->died = NULL;
    e->steps = NULL;
...
void endpoint_init(void) {
    ...
    DCHECK(g_fetch_edge.died == NULL && g_fetch_edge.steps == NULL &&
           g_xhr_edge.died == NULL && g_xhr_edge.steps == NULL,
           "a session opened with the previous session's host edge table still in hand — endpoint_free is "
    ...
    edge_reset(&g_fetch_edge);
    edge_reset(&g_xhr_edge);
    g_xhr_offered = 0;
}
...
void endpoint_record(JSContext *ctx, const char *method, JSValueConst url,
    ...
    for (int i = 0; i < g_eps_n; i++) {                 /* merge into an existing same-identity endpoint */
        if (same_identity(&g_eps[i], method, path, prov, &kvb)) {
            ...
            g_ask_merged++;
            /* …AND WHICH OF THE TWO POPULATIONS THIS MERGE WAS, READ HERE BECAUSE HERE IS THE ONLY PLACE BOTH
               FACTS EXIST. The ask's own program state is the local the door read at its entry, and the
               RECORD's is the flag its mint decided and deliberately never re-arms — so a census taken later
               holds the second and can never recover the first, and a raise anywhere else would be asking one
               of them about the wrong instant. */
            if (!ask_pre_program && g_eps[i].pre_program) g_ask_merged_pre_program++;
            goto done;
        }
    }
...
void endpoint_surface_census(long *minted, long *assets, long *emitted, long *pre_program) {
    ...
    /* AND THE ONE IDENTITY THAT SPANS THE TWO INSTRUMENTS, ASSERTED HERE BECAUSE THIS IS THE ONLY PLACE THE
       ...
       of endpoint_ask_census's: that function asserts the two PARTITIONS and the CUT, over the six numbers only it holds,
...
/* THE ASK SIDE OF THE SAME SURFACE — see endpoint.h for what it separates that the four numbers above
   ...
   SIX AND NOT FOUR, AND IT IS A SECOND CALL RATHER THAN MORE OUT-PARAMETERS ON THE WALK ABOVE. That
...
void endpoint_ask_census(long *asks, long *pre_program, long *suppressed, long *merged, long *minted,
    ...
    DCHECK(asks && pre_program && suppressed && merged && minted && merged_pre_program,
           "the @H ask census was asked for with somewhere to put fewer than six of its numbers — the two "
           "partitions and the cut are the whole point and a caller taking one of them is reading a bare count again, "
           "which for this instrument means reading a total that cannot say whether running code ever composed "
           "a request at all");
    ...
    /* AND THE TWO CONTAINMENTS OF THE CUT, ASSERTED HERE FOR THE REASON THE TWO PARTITIONS ABOVE ARE: this is
       the only place every term of either is in one hand. They are not one check written twice — the cut
       selects on TWO independent facts and each containment fires on a different way of getting it wrong.
       THE FIRST IS AGAINST THE ARM IT SITS INSIDE. It is raised only on the line after `g_ask_merged`, so a
       value above it is a second raise added outside that arm — which is exactly how a reader would come to
       subtract it from `merged` and read a negative complement as a large one. */
    DCHECKF(g_ask_merged_pre_program <= g_ask_merged,
            "the @H surface counted more asks merging into a record minted before the first program (%ld) "
            "than asks that merged at all (%ld) — the cut is raised only inside the merge arm that raises the "
            "total, so a larger cut is a raise added outside it, and a reader taking the rest of the merges "
            "as the arm where running code re-composed its OWN addresses would read a negative count as a "
            "large one", g_ask_merged_pre_program, g_ask_merged);
    /* AND THE SECOND IS AGAINST THE OTHER FACT THE CUT SELECTS ON, WHICH THE FIRST CANNOT SEE. Every member of
       this cut was asked with a program running, so it is also contained in the complement of the
       program-state partition — and that is the quantity endpoint.h offers as the separation between the two
       readings of a zero razor, so a cut exceeding it would make the row it exists to refine arithmetically
       impossible. The two sides are raised at different events (one at the door's entry, one at the merge), so
       they part company exactly when the door's program bit and the record's stop meaning one instant, which
       is the state the mint's own DCHECK guards from the other side. */
    DCHECKF(g_ask_merged_pre_program <= g_asks - g_asks_pre_program,
            "the @H surface counted more asks merging into markup-minted records (%ld) than asks made after a "
            "program had started at all (%ld of %ld) — both facts are read at the same door, the ask's from "
            "the entry and the record's from its mint, so this fires when those two stop describing one "
            "instant and the cut is then a fraction of a population the ask census does not hold",
            g_ask_merged_pre_program, g_asks - g_asks_pre_program, g_asks);
...
static void edge_declare(EndpointEdge *e, const char *who, const char *const *steps, int first_stage) {
    ...
    DCHECKF(steps != NULL && steps[0] != NULL,
            ...
            "bare count wearing a partition's shape", who);
    while (steps[n]) n++;
    /* RE-DECLARED WITH THE SAME TABLE IS THE ORDINARY CASE AND COSTS NOTHING — one session per instance, and
       a host that opens two runs this twice. A DIFFERENT table is a second machine reaching this edge's rows,
       which it may not: they are keyed by ONE machine's stages and two machines' stages summed into one
    ...
    /* ALLOCATION, SO A `CHECK` — §Offensive-programming names OOM as the universal invariant that is fatal in
       ...
       through. */
    CHECKF(e->died != NULL, "the %s edge census could not allocate its per-stage partition", who);
    e->steps = steps;
    e->stage_n = n;
...
/* …AND THE STATE IS GONE. `stage` is where it was standing, `reached` whether its construction had completed
   (the fetch edge's OFFER, the XHR edge's PLACEMENT — endpoint.h states which and why they differ). The two
...
static void edge_freed(EndpointEdge *e, const char *who, int stage, int reached) {
    int i = stage - e->stage_first;

    DCHECKF(e->steps != NULL,
            "a %s state was torn down before its machine declared its stages — the census cannot name the arm "
            "this teardown belongs to, and a partition with an unnameable arm is a total with a hole in it",
            who);
    ...
    CHECKF(i >= 0 && i < e->stage_n,
           "a %s state was torn down standing at stage %d, which its machine's own table does not name "
           ...
           who, stage, e->stage_n, e->stage_first);
    e->freed++;
    if (reached) e->freed_reached++;
    else e->died[i]++;
}
...
/* THE SAME DECLARATION FOR THE OTHER EDGE, AND THE TABLE IS `SEND_STEPS` AND NOT THE LIFECYCLE MACHINE'S.
   endpoint.h holds the refutation in full: `send()` is where an XMLHttpRequest request is CONSTRUCTED and
...
void endpoint_fetch_edge_began(void) { g_fetch_edge.began++; }
...
void endpoint_fetch_edge_offered(void) { g_fetch_edge.reached++; }

void endpoint_fetch_edge_freed(int stage, int offered) { edge_freed(&g_fetch_edge, "fetch", stage, offered); }

/* THE OTHER EDGE'S THREE, AND THE SECOND OF THEM IS A PLACEMENT AND NOT AN OFFER. `send()` hands the
   constructed request to the lifecycle machine at §3.5.6 step 12 or 13 and is then done with it, so its
   teardown can say whether the construction COMPLETED and cannot say whether an address was ever offered —
   which is why the partition below is over the placement and why the offer is a row of its own. */
void endpoint_xhr_edge_began(void) { g_xhr_edge.began++; }
void endpoint_xhr_edge_placed(void) { g_xhr_edge.reached++; }
void endpoint_xhr_edge_freed(int stage, int placed) { edge_freed(&g_xhr_edge, "xhr send", stage, placed); }

/* …AND THE ADDRESS THIS EDGE OFFERED THE SURFACE, raised on the line before `xhr_record_endpoint`'s own
   endpoint_record call. It is raised INSIDE the lifecycle machine, so it is a fact about this EDGE and about
   no send state — endpoint.h states why no inequality between it and the placement row may be asserted. */
void endpoint_xhr_edge_offered(void) { g_xhr_offered++; }
...
static long edge_stage_sum(const EndpointEdge *e) {
    ...
    for (i = 0; i < e->stage_n; i++) sum += e->died[i];
    return sum;
}

/* THE ONE IDENTITY THAT IS A FACT ABOUT EVERY EDGE'S OWN TEARDOWN, ASSERTED WHERE BOTH TERMS ARE IN ONE HAND.
   The other relations are NOT shared, and that is a finding about the two machines rather than a shape this
   helper could not express — each emitter states its own, with the reason it holds there.
   THE PARTITION. The two sides are raised in the two arms of ONE teardown, so they part company exactly when
   a third way out of that function is added without classifying the state it lets go — which is how a state
   that dies in a new way goes missing from a histogram that still sums to something plausible. It survives
   the deep-fork copy that defeats every OTHER relation here, because a copy gets its own teardown and that
   teardown raises exactly one of these two arms like any other. */
static void edge_partition(const EndpointEdge *e, const char *who, long sum) {
    DCHECKF(sum + e->freed_reached == e->freed,
            "the %s edge's torn-down states do not sum over what happened to them (%ld by the stage rows plus "
            "%ld whose construction had completed, %ld freed) — the rows are the two arms of ONE teardown, so "
            ...
            who, sum, e->freed_reached, e->freed);
}

/* ONE ARM PER STAGE, EVERY ONE EMITTED INCLUDING THE ZEROES, which is absent.c's rule for its histograms and
   is load-bearing twice over. It is what makes a stage reading 0 the positive statement that no construction
   died there, where a row listing only its non-zero arms is a table whose shape a reader has to know before
   they can tell an arm that never fired from an arm that does not exist — and extension/bridge.js REFUSES an
   empty histogram on this census by name, so a table that listed only its non-zero arms would abort the
   trusted zone for every document whose constructions all survived.
   THE KEY IS COMPUTED AND IS NOT A FIELD NAME — json_buf_key takes a literal and the compiler enforces it,
   which is what makes every FIELD name in this seam auditable; a STAGE LABEL is another component's
   declaration and no producer here could declare it, so it goes through the VALUE entry. That entry also
   ESCAPES, which this key needs and absent.c's row keys do not: these labels quote the standards' own
   `"no-cors"`, `init["body"]` and `"fetching req"`, so a raw write would end the JSON string early and the
   document embedding this census would not parse. */
static void edge_stage_hist(JsonBuf *b, const EndpointEdge *e) {
    int i;

    json_buf_raw(b, "{");
    for (i = 0; i < e->stage_n; i++) {
        if (i) json_buf_raw(b, ",");
        json_buf_str(b, e->steps[i]);
        json_buf_raw(b, ":");
        edge_num(b, e->died[i]);
    }
    json_buf_raw(b, "}");
...
char *endpoint_fetch_edge_rows(void) {
    ...
    sum = edge_stage_sum(&g_fetch_edge);
    edge_partition(&g_fetch_edge, "fetch", sum);
    /* AND THE FIRST CONTAINMENT, WHICH IS THIS EDGE'S AND NOT THE OTHER ONE'S. The two sides are raised at
       DIFFERENT events — one at the teardown, one on the line before the door — so the slack is a POPULATION
    ...
    DCHECKF(g_fetch_edge.freed_reached <= g_fetch_edge.reached,
            ...
            g_fetch_edge.freed_reached, g_fetch_edge.reached);
    ...
    DCHECKF(g_fetch_edge.reached <= g_asks,
    ...
    /* THE NAMES ARE LITERALS AT THE json_buf_key CALL AND CANNOT BE ANYTHING ELSE — the macro concatenates
       ...
       field name this seam emits is readable off this source. IT IS ALSO WHY THE TWO EDGES' EMITTERS ARE TWO
       FUNCTIONS WHILE EVERYTHING ABOVE THEM IS SHARED: a computed key is not a thing this seam can express.
    ...
    json_buf_raw(&b, ",");
    json_buf_key(&b, "epFetchAskOfferedLife");      edge_num(&b, g_fetch_edge.reached);
    json_buf_raw(&b, ",");
    json_buf_key(&b, "epFetchOutFreedLife");        edge_num(&b, g_fetch_edge.freed);
    json_buf_raw(&b, ",");
    json_buf_key(&b, "epFetchOutFreedOfferedLife"); edge_num(&b, g_fetch_edge.freed_reached);
    json_buf_raw(&b, ",");
    json_buf_key(&b, "epFetchOutDiedAtLife");
    edge_stage_hist(&b, &g_fetch_edge);
    return json_buf_take(&b);
}
...
char *endpoint_xhr_edge_rows(void) {
    ...
    /* THE CROSS-CHECK THAT HAS TO COME BEFORE THE ABSENT ARM, because the absent arm returns. An offer at this
       edge's door with its send machine never declared is core/xhr having recorded an endpoint in a session
       whose own xhr_init did not run — which would publish this edge's share of `epAsks` with no census under
       it, and would do it as an ABSENCE, which §Testing's rule says is a different fact from a zero and is the
       one a reader stops at. */
    DCHECKF(g_xhr_offered == 0 || g_xhr_edge.steps != NULL,
            "the XHR edge offered this surface %ld addresses with no send machine ever declared to it — the "
            "declaration is xhr_init's and every member of §3 runs after it, so this is a session whose "
            "endpoint records came from a component this census was never told about, and the rows are about "
            "to be ABSENT from a document that nevertheless counted this edge's asks",
            g_xhr_offered);
    if (!g_xhr_edge.steps) {
        /* NO ROWS AT ALL, WHICH IS A DIFFERENT DOCUMENT FROM SIX ZEROES AND IS THE HONEST ONE — the same
           sentence as the fetch edge's absent arm above, and for the same reason: a host that installs no
           XMLHttpRequest runs no send machine and there is no population. The EMPTY STRING and not `{}`,
           because these are ROWS of the census above and the absent form has to be the absence of the rows
           and of the comma in front of them. */
        json_buf_raw(&b, "");
        return json_buf_take(&b);
    }
    sum = edge_stage_sum(&g_xhr_edge);
    edge_partition(&g_xhr_edge, "xhr send", sum);
    /* AND THERE IS NO `freed_placed <= placed` HERE, WHICH IS A FACT ABOUT THIS MACHINE AND NOT AN OMISSION.
       A step state is BYTE-COPIED at a deep fork and the copy inherits `placed`, so two copies file against
       one placement — and unlike core/fetch, `XHR_SEND_DECL` declares NO `unforkable`, so nothing refuses
       that fork. It is not a corner either: §3.5.6's SYNCHRONOUS arm sets `placed` and then PARKS inside its
       own call to the lifecycle machine, which goes on to fire `readystatechange` and `progress` at the
       page's own listeners — page code, at a depth where the send frame is live and forkable. An assert here
       would therefore fire on a legitimate state, which is the concession shape §Offensive-programming
       refuses, and the row pair is read as two facts rather than subtracted. */
    /* THE CONTAINMENT THIS EDGE DOES HAVE IS OVER THE OFFER ROW AND NOT OVER THE PLACEMENT — the placement is
       one machine short of the door and endpoint.h states why nothing between them may be asserted. Every
       offer is raised on the line before an endpoint_record call that raises `g_asks` before its own gate, so
       this edge's offers are contained in the surface's asks and the slack is every OTHER door. */
    DCHECKF(g_xhr_offered <= g_asks,
            "the XHR edge offered this surface more addresses (%ld) than the surface counted asks at its door "
            "(%ld) — the offer is raised on the line before endpoint_record and that door raises the ask "
            "total before its own gate, so a larger offer count is an offer raised with no record call under "
            "it, and a reader taking the rest of the asks as the OTHER doors' share would read a negative "
            "count as a large one",
    ...
    json_buf_raw(&b, ",");
    json_buf_key(&b, "epXhrAskPlacedLife");       edge_num(&b, g_xhr_edge.reached);
    json_buf_raw(&b, ",");
    json_buf_key(&b, "epXhrAskOfferedLife");      edge_num(&b, g_xhr_offered);
    json_buf_raw(&b, ",");
    json_buf_key(&b, "epXhrOutFreedLife");        edge_num(&b, g_xhr_edge.freed);
    json_buf_raw(&b, ",");
    json_buf_key(&b, "epXhrOutFreedPlacedLife");  edge_num(&b, g_xhr_edge.freed_reached);
    json_buf_raw(&b, ",");
    json_buf_key(&b, "epXhrOutDiedAtLife");
    edge_stage_hist(&b, &g_xhr_edge);
    return json_buf_take(&b);
}
...
void endpoint_free(void) {
    ...
    /* …AND THE ASK COUNTERS BESIDE THE ARRAY THEY ARE A FACT ABOUT. One process runs many sessions and this is
       ...
       reading that says running code composed a request. The mint arm is reset with `g_eps_n` for a
    ...
    edge_release(&g_fetch_edge);
    edge_release(&g_xhr_edge);
    g_xhr_offered = 0;
}

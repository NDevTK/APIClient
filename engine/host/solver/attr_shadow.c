...
typedef struct { lxb_dom_element_t *el; int kind; char *name; JSValue opaque; } AttrShadow;
static AttrShadow *g_attr_shadow = NULL; static int g_attr_shadow_n = 0, g_attr_shadow_cap = 0;

int attr_shadow_find(lxb_dom_element_t *el, int kind, const char *name) {
    for (int i = 0; i < g_attr_shadow_n; i++)
        if (g_attr_shadow[i].el == el && g_attr_shadow[i].kind == kind && strcmp(g_attr_shadow[i].name, name) == 0) return i;
    return -1;
}
void attr_shadow_set(JSContext *ctx, lxb_dom_element_t *el, int kind, const char *name, JSValueConst opaque) {
    int i = attr_shadow_find(el, kind, name);

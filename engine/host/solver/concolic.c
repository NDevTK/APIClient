 ...
 *   program. No derivation ever CHANGES one or invents one, because nothing a derivation does can change how
 *   the attacker got the bytes there: `location.hash.slice(1)` is no longer the fragment for INJECTION (the `#`
 *   is gone, and concolic_lead_hook says so) and is still the fragment for DELIVERY, which is what a
 *   reproduction envelope and a percent-encode set are questions about.
 *   WHAT A DERIVATION DOES IS UNION THEM, over the roots of its OPERANDS (derived_root_join). This paragraph
 *   read "INHERITED unchanged through EVERY derivation" and that is true of a derivation over ONE unknown, which
 *   is the ordinary case and composes to that operand's own root byte for byte. It was false of the others and
 *   silently: `location.hash + location.search` took the FIRST operand's and reported the fragment's
 *   percent-encode set and the fragment's address component as the whole constraint on bytes that were half in
 *   the query — a WRONG report by §@S rather than a partial one, generated by the hottest path in the solver.
...
/* MEMBER `i` OF A ROOT — DEFINED WITH THE REGISTRY BELOW, DECLARED HERE BECAUSE THE PIN IS ITS FIRST
   READER. The separator and the walk were already moved up once, past the mint that composes a joint, because
   the registry became their second reader; the mark below is the third and sits above both. A forward
   declaration rather than a third move, so the walk stays beside the registry it is stated in terms of. */
...
void concolic_pin(const char *src, const char *root, ConcolicLit kind, const char *val, JSConcolicEqOp algo) {
    ...
    /* MARKED ON EVERY MEMBER, BECAUSE A ROOT MAY NAME A SET AND THE QUESTION IS ASKED OF THE MEMBERS.
       concolic_principal_pinned walks the DECLARED principals and looks each one up BY ITS OWN NAME, so a mark
       written under a joint key would leave `message.origin`'s own entry unpinned for a flow that pinned a
       value derived from it — and §Attacker-sources' unforgeable-origin rule would then answer "this flow
       demanded no principal" for a flow that demanded one. That direction SUPPRESSES nothing and emits a PoC
       no cross-origin attacker can deliver, which is the one outcome §Attacker-sources names as forbidden.
       IT IS A LOOP AND NOT A TEST FOR JOINTNESS: root_member answers at i == 0 and stops for a single source,
       so the ordinary case walks once and there is no second spelling of it. `cons_entry` may grow the head,
       so nothing here holds its result across another call to it. */
    if (root) {
        const char *mp;
        size_t mn;
        int mi;

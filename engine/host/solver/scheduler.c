...
#include "solver/fork.h"           /* fork_spawn_sibling — build the snapshot-fork sibling requested at suspend */
...
int reg_readd(JSContext *ctx, Flow f)   /* re-queue a Flow (exposed: fork.c re-queues the snapshot sibling) */
{
...
void scheduler_run(JSContext *ctx)
{
    ...
    for (;;) {
        ...
        if (st == 1) {
            ...
            f.dec_n = g_dec_n;
            fork_spawn_sibling(ctx, &f);   /* SNAPSHOT-FORK: if this flow suspended at an opaque gate, build the sibling that continues from a frame snapshot + the shared frozen deltas (fork.c); no-op otherwise */
            g_cur_flow = NULL;
            reg_readd(ctx, f);
        } else if (st == 2) {

 ...
 * they all are.
 *
 * WHERE THE EXPLICIT VALUE LIVES. §2.6.1's "explicitly set attr-element(s)" and its cached FrozenArray are per
 * ELEMENT and per MEMBER, and they are JS VALUES on the element's wrapper under a Symbol this component minted
...
static int g_id_el_get[ARIA_EL_N], g_id_el_set[ARIA_EL_N];

/* ---- §2.6.1's per-element, per-member state ---------------------------------------------------------------
...
static JSValue aria_record(JSContext *ctx, JSValueConst el, int magic, bool create)
{
    JSValue map, rec;

    DCHECK(magic >= 0 && magic < ARIA_EL_N,
           "an ARIAMixin element reflection asked for state with a magic the mixin does not declare");
    ...
    rec = JS_GetPropertyStr(ctx, map, ARIA_ELS[magic].attr);
    if (!JS_IsObject(rec)) {
        ...
        JS_SetPropertyStr(ctx, map, ARIA_ELS[magic].attr, JS_DupValue(ctx, rec));
    }
    JS_FreeValue(ctx, map);
    return rec;
}
...
static JSValue aria_explicit(JSContext *ctx, JSValueConst el, int magic)
{
    JSValue rec = aria_record(ctx, el, magic, false), v;
...
static void aria_set_explicit(JSContext *ctx, JSValueConst el, int magic, JSValue v)
{
    JSValue rec = aria_record(ctx, el, magic, true);
...
static JSValue aria_frozen(JSContext *ctx, JSValueConst el, int magic, JSValue list)
{
    JSValue rec = aria_record(ctx, el, magic, true), cached;

    DCHECK(ARIA_ELS[magic].plural, "a singular element reflection asked for §3.2.27's frozen array");
...
static JSValue aria_from_explicit(JSContext *ctx, JSValueConst this_val, int magic, JSValue set)
{
    lxb_dom_node_t *self = lxb_dom_interface_node(element_of_value(this_val));
    JSValue out;
    uint32_t n = 0, i, kept = 0;

    if (!ARIA_ELS[magic].plural) {
    ...
    JS_FreeValue(ctx, set);
    return aria_frozen(ctx, this_val, magic, out);
}
                               ...
                               JSValue cb_result, JSValue *presult, JSValue **out_cb, int *out_argc)
{
    ...
    if (hdr->stage == ARIA_EL_START) {
        ...
        *presult = JS_NULL;
        ...
        set = aria_explicit(ctx, hdr->this_val, magic);
        if (!JS_IsUndefined(set)) {
            *presult = aria_from_explicit(ctx, hdr->this_val, magic, set);
        ...
        val = element_attr_get(ctx, hdr->this_val, ARIA_ELS[magic].attr);
        if (!val) return JS_STEP_DONE;
        len = strlen(val);
        s->idscap = len + 1;
        ...
        if (s->ntok == 0) {
            ...
            *presult = ARIA_ELS[magic].plural ? aria_frozen(ctx, hdr->this_val, magic, aria_collect(ctx, s))
                                              : JS_NULL;
            return JS_IsException(*presult) ? JS_STEP_ABRUPT : JS_STEP_DONE;
        }
    ...
    *presult = aria_frozen(ctx, hdr->this_val, magic, aria_collect(ctx, s));
    return JS_IsException(*presult) ? JS_STEP_ABRUPT : JS_STEP_DONE;
}
...
static JSValue js_aria_element_set(JSContext *ctx, JSValueConst this_val, JSValueConst val, int magic)
{
    ...
    if (JS_IsNull(val)) {
        ...
        aria_set_explicit(ctx, this_val, magic, JS_UNDEFINED);
        dom_cow_remove_attribute(el, ARIA_ELS[magic].attr);
        return JS_UNDEFINED;
    }
    ...
    DCHECK(ARIA_ELS[magic].plural ? (JS_IsArray(val) || concolic_is(val)) : element_of_value(val) != NULL,
    ...
    /* §2.6.1's ORDER, and it is load-bearing: the content attribute is set to the EMPTY STRING first and the
       explicit value recorded after, because the write runs the attribute change steps below, which DROP
       whatever was explicitly set. Recording first would erase the assignment being made. */
    element_attr_set(ctx, this_val, ARIA_ELS[magic].attr, "");
    aria_set_explicit(ctx, this_val, magic, JS_DupValue(ctx, val));
    return JS_UNDEFINED;
}
...
void aria_mixin_init(JSContext *ctx)
{
    ...
    for (i = 0; i < ARIA_EL_N; i++) {
        g_id_el_get[i] = idl_getter_id_step(ctx, &ARIA_EL_STEP, i);
        /* `Element?` and `FrozenArray<Element>?` — the NULLABILITY is the type's, so `el.ariaOwnsElements = 3`
           is a TypeError from the declaration and `= null` reaches the body as the IDL null. A FrozenArray is
           §3.2.27's frozen `sequence<T>`, so the value the setter takes is converted as one. */
        g_id_el_set[i] = idl_setter_id(ctx, ARIA_ELS[i].plural ? IDL_SEQUENCE_INTERFACE_NULLABLE
                                                               : IDL_INTERFACE_NULLABLE,
                                       false, js_aria_element_set, i);
        /* WHAT "Element" MEANS TO THIS ENGINE: every node wrapper is ONE class, so the class id says "a Node"
           and the narrowing is what says "an Element" — the same pair §4.13.7.3's `HTMLElement anchor` uses,
           and the reason `el.ariaOwnsElements = [document]` is a TypeError from the TYPE. */
...
void aria_mixin_install(JSContext *ctx, JSValueConst proto)
{
    ...
    for (i = 0; i < ARIA_EL_N; i++)
        idl_install_accessor_step(ctx, proto, ARIA_ELS[i].idl, g_id_el_get[i], g_id_el_set[i]);
}

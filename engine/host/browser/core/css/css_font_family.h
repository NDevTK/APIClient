 ...
 * a `<family-name>` yet: the cascade carries a specified value as TEXT (core/css/css_computed_value.h), and
 ...
 *
 * A SINGLE IDENTIFIER THAT IS A GENERIC KEYWORD IS THE `<generic-family>` ARM AND NOT AN EXCLUDED
 * `<custom-ident>`, which is why the arm is chosen before the exclusion is asked. §2.1's value is
 * `[ <family-name> | <generic-family> ]#` and `<generic-family>` is the alternative that matches `serif`; the
 * exclusion exists to stop the OTHER alternative claiming it. Asking the exclusion first would make
 * `font-family: serif` invalid.
 *

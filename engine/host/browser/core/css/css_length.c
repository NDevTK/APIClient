...
static CssPx css_len_unit_px(JSContext *realm, const CssFontMetrics *font, const char *unit, double num)
{
    ...
    if (css_len_is_viewport(unit))
        ...
        DFAIL("a length in one of css-values §6.1.2.1's SMALL, LARGE or DYNAMIC viewport-percentage units "
              ...
              "today and is still a separate fact for the same reason. AND THAT INSTRUCTION IS THE SECOND "
              "SUBPROBLEM RATHER THAN THE FIRST, BECAUSE THIS CRASH CANNOT FIRE FOR A STYLESHEET "
              "DECLARATION AT ALL: lexbor's `lxb_css_property_state_length` resolves a dimension token "
              "through `lxb_css_unit_absolute_relative_by_name` and RETURNS FALSE when that answers NULL, "
              "which DROPS the declaration, so the cascade never carries the value and this arm is never "
              "asked about it. Lexbor's tables (css/unit/const.h, css/unit/res.h) carry none of the small, "
              "large or dynamic families and none of the container units, while `css_length_is_length_unit` "
              "below admits every one of them — ONE FACT IN TWO TABLES, and the loser DROPS IN SILENCE "
              "rather than crashing. Derive today's gap rather than trusting a number: diff the unit "
              "identifiers in lexbor's absolute and relative enums against the length families listed in "
              "core/css/css_namespace.c. MEASURED on a frozen native binary over a three-arm fixture, with "
              "controls both ways: a `height` of `100dvh`, of `100cqh`, and of an INVENTED `100zzq` all "
              "abort identically at core/layout/block_flow.c's flex `column` arm — which is reached ONLY "
              "when the height BEHAVES AS AUTO — while `100vh` and `50em` abort somewhere else entirely. A "
              "unit lexbor does not know is INDISTINGUISHABLE FROM `auto`. So until a dimension in one of "
              "these units survives the parse there is nothing for the viewport facts named above to be "
              "asked BY, and a reader who builds them first changes no page. The table is a GENERATED "
              "perfect hash whose generator is NOT in this tree, so extending it by hand is a "
              "silent-corruption risk and the diff to prefer derives lexbor's set from the one this file "
              "already owns. RETIREMENT: this paragraph goes when a dimension in one of these units REACHES "
              "this arm, i.e. when that fixture aborts here rather than in block_flow.c");

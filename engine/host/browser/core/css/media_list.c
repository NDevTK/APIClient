...
void media_list_init(JSContext *ctx)
{
    ...
    agent_state_class("element", &g_list_class, "CSSOM §4.4 \"The MediaList Interface\"'s class");
...
void media_list_free(JSRuntime *rt)
{
    ...
    /* THE CLASS ID IS NOT RESET HERE. It is declared under `element`, whose release ends in
       ...
       back: element_free calls media_list_free, and this says so.
       AND THE ID WAS THE ONE THING THIS RELEASE DID NOT GIVE BACK, while everything beside it did:
       g_atom_queries, g_queries_key and the pool entries above are reset here and the class was not, so a
       SECOND agent in one process met the init's own latch on a stale id, returned at once, and
       every MediaList of that agent would have been keyed by JS_ATOM_NULL and served by pool entry -1.

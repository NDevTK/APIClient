...
IntrinsicInlineSizes flex_intrinsic_inline_sizes(lxb_dom_element_t *el)
{
    ...
    DFAILF("%s: css-flexbox-1 §9.9.1 \"Flex Container Intrinsic Main Sizes\" is what this container's INLINE "
           ...
           "(1) THE OUTER PREFERRED SIZE, WHOSE ARITHMETIC IS BUILT AND WHOSE COMPOSITION IS NOT — and this "
           "clause used to say the term itself was missing and that ONE diff retired both crashes, which was "
           "true when it was written and is the half a reader must not act on now. CSS 2.1 §10.4 \"Minimum "
           "and maximum widths: 'min-width' and 'max-width'\"'s clamp and css-sizing-3 §3.3 \"Box Edges for "
           "Sizing: the box-sizing property\"'s conversion over a declared `width` are core/layout/"
           "intrinsic_size.c's `is_declared_inline_sizes`, and that file no longer crashes. WHAT IS LEFT IS A "
           "DIFFERENT COMPOSITION OF THE SAME TERM, and the difference is the whole of the remaining work: "
           "css-sizing-3 §5.2 \"Intrinsic Contributions\" makes a declared size REPLACE the measured one — a "
           "hypothetical float containing a `width: 500px` box is 500px wide — while §9.9.3 takes \"the larger "
           "of its outer min-content size and outer preferred size\", a MAXIMUM, so an item whose text is "
           "wider than its declared `width` contributes the text here and the declaration there. So the next "
           "diff EXPORTS the per-property half of that term (the §3.3 conversion plus §10.4's clamp, which is "
           "`static` today because §5.2 was its only consumer) and composes §9.9.3's maximum over it; it does "
           "NOT reuse `is_declared_inline_sizes` itself, which would report §5.2's replacement as §9.9.3's "
           "maximum. §9.9.3's own \"if that is not an automatic size\" is the arm that makes an `auto` width "
           "contribute nothing here, and css-sizing-3 §3.2 is where \"specifies an automatic size\" is said. "
           ...
           "reaches, \"If the item has a definite used flex basis, that's the flex base size\", and the "
           "`flex-basis` value `auto` is what routes between them. IT IS DEFINED IN §7.1 \"The flex "
           "Shorthand\" AND NOT IN §7.2.3 \"The flex-basis property\", WHICH IS WHERE THIS CLAUSE USED TO "
           "SEND ITS READER: §7.1 carries the <'flex-basis'> value list, §7.2.3 carries the property's own "
           "definition and refers back to the values \"defined above\". The wrong number was invisible to "
           "engine/citegen.mjs because the citation named no standard, so its quotation channel refused to "
           "judge a section only a file vote had placed — the shielding that tool's own output names, and the "
           "reason this one now writes `css-flexbox-1` in front of the number. The sentence, whose OPENING "
           "CONDITION is the half a trimmed quotation would drop, is css-flexbox-1 §7.1 \"The flex "
           "Shorthand\"'s: \"When specified on a flex item, the auto keyword retrieves the value of the main "
           "size property as the used flex-basis. If that value is itself auto, then the used value is "
           "content.\" "

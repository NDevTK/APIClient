...
IntrinsicInlineSizes flex_intrinsic_inline_sizes(lxb_dom_element_t *el)
{
    ...
    DFAILF("%s: css-flexbox-1 §9.9.1 \"Flex Container Intrinsic Main Sizes\" is what this container's INLINE "
           ...
           "WHAT IS MISSING IS ONE ITEM'S CONTRIBUTION AND, UNDER IT, ONE PROPERTY LAYER. §9.9.3 \"Flex Item "
           "Intrinsic Size Contributions\" states the contribution as \"the larger of its outer min-content "
           "size and outer preferred size if that is not an automatic size\", then: \"each contribution is "
           "capped by the item's flex base size if the item is not growable, floored by the item's flex base "
           "size if the item is not shrinkable, and then further clamped by the item's min/max main size.\" "
           "The outer sizes and the preferred size are answerable now; the cap and the floor are not, because "
           "they read THREE PROPERTIES THIS CASCADE DERIVES NO COMPUTED VALUE FOR — §7.2.1 \"The flex-grow "
           "property\", §7.2.2 \"The flex-shrink property\" and §7.2.3 \"The flex-basis property\" — and the "
           "\"non-collapsed\" in §9.9.1.2's own sentence reads a fourth, css-display-3 §4 \"Invisibility: the "
           "visibility property\", which §4.4 \"Collapsed Items\" names by its one declaration. "
           "SO BUILD THAT LAYER FIRST, in the two places §5.1's and §5.2's were built in: each property's "
           "`Computed value:` line in core/css/css_computed_value.c, and §7.1 \"The flex Shorthand\"'s row in "
           "core/css/css_shorthand.c. THE SHORTHAND IS NOT OPTIONAL AND IS THE HARD HALF — §7.1's grammar is "
           "`none | [ <'flex-grow'> <'flex-shrink'>? || <'flex-basis'> ]`, an ORDERED PAIR inside a `||` whose "
           "omitted components take defaults that are NOT their longhands' initial values (\"When omitted, it "
           "is set to 1\" for both factors, \"When omitted from the flex shorthand, its specified value is 0\" "
           "for the basis), so neither direction of core/css/css_shorthand.c's CSS_SH_ALL_OF is its rule. "
           "`css_computed_models` and `css_shorthand_complete_for` are the two predicates that say whether it "
           "is there yet. "
           "THEN §9.2 \"Line Length Determination\"'s STEP 3 IS THE FLEX BASE SIZE, and its THIRD arm is the "
           "one every call reaching this line is inside — \"If the used flex basis is content or depends on "
           "its available space, and the flex container is being sized under a min-content or max-content "
           "constraint\" — whose answer is stated in the next sentence: \"The flex base size is the item's "
           "resulting main size.\" Under that arm the cap and the floor are arithmetic over the same number "
           "this component already measures, so the property layer is what unblocks the common case and §9.2's "
           "aspect-ratio arm is not on the path",
           box_subject(el, nbuf, sizeof nbuf));
    return out;
}

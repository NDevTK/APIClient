...
static void lb_child(LbRun *r, lxb_dom_element_t *parent, lxb_dom_node_t *n)
{
    ...
    if (atomic && !inline_block)
        DFAIL("CSS 2.2 §9.2.2 makes this an ATOMIC INLINE-LEVEL box, which \"participate[s] in [its] inline "
              ...
              "'vertical-align: baseline' is used to set the baseline of the row\", and NO COMPONENT ANSWERS "
              "§17.5.3 — core/layout/used_value.c's `height: auto` and declared-`height` arms both crash "
              "naming it. So BUILD §17.5.3, and this box's `below` term is its first row's; until then this "
              "crash is what an `inline-table` on a line reaches. AN `inline-flex` AND AN `inline-grid` NEED A "

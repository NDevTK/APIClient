...
static void lb_child(TextRunMeasure *m, lxb_dom_element_t *parent, lxb_dom_node_t *n)
{
    ...
    if (atomic && !inline_block)
        DFAIL("CSS 2.2 §9.2.2 makes this an ATOMIC INLINE-LEVEL box, which \"participate[s] in [its] inline "
              ...
              "answers that, so what is left under the sentence is §17.5.2's used inline size and the row "
              "whose baseline it names. AN `inline-flex` AND AN `inline-grid` NEED A BASELINE THEIR OWN MODULE "
                                 ...
                                 LineBoxFragment **out)
{
    ...
    DCHECK(lb_computed_is(el, "display", "inline") || lb_computed_is(el, "display", "inline-block"),
           ...
           "an item for them: an `inline-table` has no used inline size (CSS 2.1 §17.5.2; its box structure is "
           "core/layout/table_box.h's) and an `inline-flex` or "

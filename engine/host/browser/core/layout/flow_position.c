...
static void fp_require_placeable(lxb_dom_element_t *el)
{
    ...
    if (inline_level)
        DFAILF("%s: "
              ...
              "SPLIT of its margin box at the baseline named below, EXCEPT for the one box whose baseline IS "
              "that bottom margin edge, which needs no split and only that entry's own `display` test widened "
              "to admit it. THE RUN ITEM IS BUILT TOO AND IS NO "
              ...
              "already, AND FOR ONE `inline-block` AS WELL. WHAT IS LEFT DEPENDS ON WHICH BOX THIS IS, AND FOR "
              "ONE OF THEM IT IS NOT A BASELINE AT ALL. CSS 2.2 §10.8.1 \"Leading and half-leading\" — the "
              "section the standard writes the `vertical-align` definition under — states an `inline-block`'s "
              "baseline in ONE sentence WITH ITS EXCEPTION: \"The baseline of an 'inline-block' is the "
              ...
              "baseline is the bottom margin edge.\" That last clause is an ANSWER, so core/layout/line_box.c puts "
              "such a box on the line with its whole margin box above the baseline, exactly as it does a "
              "replaced element's — and for THAT box what this crash owes is only the PLACEMENT below, not a "
              "baseline. FOR EVERY OTHER ATOMIC INLINE THE BASELINE IS STILL THE WORK: an `inline-block` whose "
              "`overflow` computes to `visible` needs the sentence's main arm, its last line box, plus the \"no "
              "in-flow line boxes\" test that chooses between the two; an `inline-table` needs a different "
              "sentence of the same definition (\"The baseline of an 'inline-table' is the baseline of the "
              "first row of the table\"), which is CSS 2.1 §17.2 \"The CSS table model\"'s box structure "
              "first. For those, §10.8's step 1 splits the margin box at that line rather than putting all of "
              "it above the baseline the way `lb_atomic_extent` does for a box that has none. BUILD the inner "
              "baseline and "

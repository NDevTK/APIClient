...
/* ---- THE TABLE'S OWN DECLARED HEIGHT, WHICH §17.5.3 MAKES A MINIMUM ---------------------------------------
   ...
   A PERCENTAGE THAT RESOLVES CRASHES rather than being read, and the reason is §17.4's box split: a percentage
   height resolves against the CONTAINING BLOCK's height, and the table box's containing block is the WRAPPER —
   an anonymous box no element in this tree names, whose own height is §10.6.3's over this very sum. Reading the
   table element's block-container ANCESTOR instead would resolve the percentage against a different rectangle
   and report a real height of the wrong box. */
    ...
    *out = css_px(0.0);
    ...
    if (h.kind != CSS_LENGTH_ABSOLUTE)
        DFAILF("%s: a table box whose `height` is a PERCENTAGE that RESOLVES — css-sizing-3 §3.2.1 "
               ...
               "and CSS 2.1 §17.4 Tables in the visual formatting model says which box that is: the table "
               "generates a wrapper that \"contains the table box itself and any caption boxes (in document "
               "order)\", so the table box's containing block is that WRAPPER — an ANONYMOUS box no element in "
               "this tree names, and the identical box `used_value_containing_block`'s own table arm crashes "
               "for. RESOLVING IT AGAINST THE TABLE ELEMENT'S BLOCK-CONTAINER ANCESTOR IS NOT AN "
               "APPROXIMATION OF IT: that is the WRAPPER's containing block and not the wrapper, so the "
               "percentage would be taken of a rectangle one level too far out. BUILD the wrapper as a box "
               "§10.1 can name, and note that its own height is §10.6.3's over this sum — so the day it "
               "exists, a percentage here is also the circular case §10.5 leaves undefined and this arm "
               "becomes a decision rather than a lookup",
               box_subject(table, nbuf, sizeof nbuf));
...
void table_heights(lxb_dom_element_t *table, const TableGrid *grid, TableUsedHeights *out)
{
    ...
    for (i = 0; i < grid->ncells; i++) {
        ...
        DCHECK(c->row + c->rowspan <= nrows,
               ...
               "past the end is that shortening having been lost between the placement and this read. NOTE "
               "WHAT THIS DOES NOT CHECK: that sentence binds a cell to its ROW GROUP as well as to the "
               "table, and core/layout/table_grid.h reports no group per cell — a `rowspan` reaching out of "

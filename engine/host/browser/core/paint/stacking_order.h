...
/* DOES `el` GENERATE A BOX AT ALL — css-display-3 §2.5 "Box Generation: the none and contents keywords",
   whose two keywords are the whole of the question: `none` is "The element and its descendants generate no
   boxes or text sequences", and `contents` is "The element itself does not generate any boxes, but its
   children and pseudo-elements still generate boxes and text sequences as normal". The two are ONE answer
   here because §9.9.1's subject is a box and neither keyword leaves one for this element.

   IT IS EXPORTED BECAUSE IT IS THE PRECONDITION EVERY OTHER ENTRY IN THIS HEADER STATES AND NONE OF THEM
   OFFERED A WAY TO ASK. `stacking_context_forms`, `stacking_level`, `stacking_context_of`,
   `stacking_layer_of` and `stacking_order_compare` each crash for an element that generates no box — the
   last of them says so in this header's own words, "must each generate a box" — so a caller obeying that
   sentence had nothing to call, and its only two courses were to spell css-display-3 §2.5 a second time or
   to find out by aborting. CLAUDE.md names the first as the shape that drifts and the second as a contract
   that names a hazard and offers no exit from it; this is the exit, and it is a SEPARATE ENTRY rather than
   an out-parameter on one of the five so that a caller asking it cannot read the answer as any of theirs.
   It is the SAME function those five aborts test, not a second opinion agreeing with them today.

   IT ANSWERS AND DOES NOT CRASH, which is what makes it usable as a filter: a walk over a subtree meets
   boxless elements as an ordinary matter — css-display-3 §2.5's own `contents` sentence keeps such an
   element's CHILDREN in the box tree — so a caller must be able to step over one without an abort, and the
   five entries above stay loud for the element a caller has already decided has a box. */
...
/* THE ANSWER THIS COMPONENT EXISTS FOR, and the consumer of everything above it: does `a`'s box paint BEFORE
   ...
   BOTH ELEMENTS MUST BE IN ONE DOCUMENT and must each generate a box — `stacking_generates_box` above is how
   a caller asks the second, and it is named here because a precondition with no entry behind it is one a
   caller can only discover by aborting. A caller mixing two documents is asking a question CSS 2.1 does not
   define, and it crashes rather than inventing an order between two canvases.

   THIS IS THE ANSWER FOR HIT-TESTING AND OCCLUSION, which core/paint/paint_order.h states and which is worth
   repeating at the entry rather than at the component that is NOT it: CSS 2.1 §E.2's order is over MARKS and
   one box contributes several, so a consumer asking WHICH BOX IS IN FRONT reads this comparator and never
   that walk. WHAT SUCH A CONSUMER MUST NOT EXPECT FROM THE STANDARD is a definition of the hit test itself.
   CSSOM VIEW §5 "Extensions to the Document Interface" spends `elementFromPoint`'s step 2 on "a box in the
   viewport that would be a target for hit testing at coordinates x, y" and then declines to say what that is:

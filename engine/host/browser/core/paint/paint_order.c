/* CSS 2.1 §E.2 "Painting order" — the sequence a stacking context's boxes are offered in. See paint_order.h
   ...
   for the two residuals that name the sub-list enumerations this file does not sequence. */
#include <stdbool.h>
#include <stddef.h>
#include <stdlib.h>
#include <string.h>
...
{
    /* THE ROOT ELEMENT IS ANSWERED HERE AND NOT BY THE PARTITION, because §9.9.1's layers are a partition of a
       context's DESCENDANTS and core/paint/stacking_order.c crashes by name when asked for the root's — "the
...
static void po_collect(JSContext *ctx, lxb_dom_element_t *context_el, PoSet set, PoList *out)
{
    ...
    for (n = shadow_root_next_in_shadow_including(ctx, root, root); n != NULL; ) {
        ...
        positioned = layer == STACKING_LAYER_NEGATIVE || layer == STACKING_LAYER_LEVEL_ZERO ||
...
static bool po_takes_step_2(lxb_dom_element_t *ce)
{
    ...
    DCHECKF(!table_box_kind_is_internal(kind),
            ...
            "been taken out of. BUILD the resolution from css-tables-3, which is where the interaction between "
            "a positioned table-internal box and CSS 2.1 §17.5.1's layers is stated; a guess here either paints the "
            "background twice or never offers it at all",
...
static bool po_walk(JSContext *ctx, lxb_dom_element_t *ce, bool pseudo, PaintOrderVisit visit, void *user)
{
    ...
    /* CSS 2.1 §E.2 step 2 — the context element's own background and border. */
    if (po_takes_step_2(ce) && !visit(PAINT_STEP_CONTEXT_BOX, ce, user)) return false;
    ...
    for (i = 0; ok && i < blocks.n; i++) ok = visit(PAINT_STEP_DESCENDANT_BOX, blocks.v[i], user);

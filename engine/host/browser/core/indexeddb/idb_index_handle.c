...
void idb_index_handle_init(JSContext *ctx)
{
    ...
    /* AND THE CLASS ID — core/agent_state.h, which this component has told about every slot but
       this one. §4.6 "The IDBIndex interface"'s class is registered in THIS runtime, so an id
       ...
       identity aborts on. */
    agent_state_class("idb_index_handle", &g_ix_class, "§4.6's IDBIndex class");
...
void idb_index_handle_free(JSRuntime *rt)
{
    ...
    /* THE HAND-RESET LIST THAT STOOD HERE IS GONE RATHER THAN EXTENDED. agent_state_undo puts back
       ...
       and its kind, so the list this function kept in step with idb_index_handle_init's
    ...
    agent_state_undo("idb_index_handle");
}

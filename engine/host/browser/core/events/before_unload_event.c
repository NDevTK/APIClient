...
void before_unload_event_init(JSContext *ctx)
{
    ...
    agent_state_flag("event", &g_ready,
                     "HTML §7.2.7.7 The BeforeUnloadEvent interface's declaration latch");
    agent_state_class("event", &g_bue_class,
                      "HTML §7.2.7.7 The BeforeUnloadEvent interface's class, held for its per-realm proto slot");
    agent_state_value("event", &g_key,
                      "the private Symbol HTML §7.2.7.7 The BeforeUnloadEvent interface's one slot hangs off");
    agent_state_id("event", &g_id_return_value_set,
                   "HTML §7.2.7.7 The BeforeUnloadEvent interface's `returnValue` setter");
...
void before_unload_event_free(JSRuntime *rt)
{
    /* NOT `if (!g_ready) return;`. core/events/event.c's event_init calls this component's init on the ONE
       ...
       latch set. */
    DCHECK(g_ready, "HTML §7.2.7.7 The BeforeUnloadEvent interface was released in an agent that never declared it "

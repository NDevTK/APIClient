...
static JSValue js_port_deliver(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv)
{
    ...
    if (JS_IsException(data)) {
        ...
        ev = message_event_new(ctx, "messageerror", JS_UNDEFINED, "", JS_UNDEFINED);
    } else {
        ev = message_event_new(ctx, "message", data, "", JS_UNDEFINED);
...
static JSValue js_port_post(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv, int magic)
{
    ...
    /* §9.4.2 step 2: a transfer list naming THIS port is a DataCloneError, and so — while transfer is not
       built — is any non-empty list, because nothing in it can be transferred. The second argument is either
       a sequence or a StructuredSerializeOptions whose `transfer` member is one. */
    if (JS_IsObject(opts)) {
        JSValue list = JS_IsArray(opts) ? JS_DupValue(ctx, opts) : JS_GetPropertyStr(ctx, opts, "transfer");
        int64_t n = 0;
        ...
        if (JS_IsObject(list)) {
            ...
            if (JS_ToInt64(ctx, &n, len) < 0) { JS_FreeValue(ctx, len); JS_FreeValue(ctx, list); return JS_EXCEPTION; }
            JS_FreeValue(ctx, len);
        }
        JS_FreeValue(ctx, list);
        if (n != 0)
            return JS_ThrowDOMException(ctx, "DataCloneError",
                                        "this engine cannot transfer, so a transfer list can only name values "
                                        "that are not transferable");
    }

    /* §9.4.2 step 5: SERIALIZE NOW. The throw belongs to this call, not to the delivery task — a page's
       try/catch around postMessage is where the standard puts the DataCloneError. It happens even when there
       is no target, which is why it precedes the null check. */
    if (structured_serialize(ctx, argc > 0 ? argv[0] : JS_UNDEFINED, &sd) < 0)
        return JS_EXCEPTION;

    /* §9.4.2 step 6: a port with nothing on the other end posts into nowhere, and that is not an error. */
    target = d->entangled;

...
/* §4.6's MIME TYPE GROUPS. Only the groups this engine's algorithms branch on are here — each arrives with the
   ...
   (browser_process/network/mime_sniff.c, which links into the BROWSER-PROCESS artifact alone, so a
   renderer-side caller does not link — see its header); solver/reply_decode.c

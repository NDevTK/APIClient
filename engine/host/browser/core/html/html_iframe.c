        ...
        DFAILF("%s read an `<iframe>`'s `src` and it holds UNKNOWN EXTERNAL INPUT `%s`, and %s. This is the "
               ...
               has_example
                 ? "BUILD: a child navigable whose ADDRESS carries an EXAMPLE AND A DOMAIN, so the create's "
                   ...
                   "`window.open`, reached from the element side. THE DOCUMENT HALF ALREADY EXISTS AND MUST "
                   "NOT BE REBUILT: core/dom/document.h's document_set_url takes a VALUE, document_url_value "
                   "answers with one, and document_address_example is the one place a value is spent for its "
                   "bytes — §7.2.5's `pushState` arrives through it today. WHAT IS MISSING HERE IS THE ROUTE "
                   "TO IT: this function answers OWNED BYTES and navigable_create takes a `const char *src`, "
                   "so the value is destroyed at this line and again at that signature. Carry it through both "
                   "— destination as a value, the loader spending it for its fetch and handing the same value "
                   "to the child Document's install — and this crash goes with them."

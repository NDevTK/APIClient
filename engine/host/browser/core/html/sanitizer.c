...
static void sanitizer_install_realm(JSContext *ctx)
{
    ...
    idl_install_method(ctx, proto, "allowElement", g_id_allow_element);
    idl_install_method(ctx, proto, "removeElement", g_id_remove_element);
    idl_install_method(ctx, proto, "replaceElementWithChildren", g_id_replace_with_children);
    idl_install_method(ctx, proto, "allowProcessingInstruction", g_id_allow_pi);
    idl_install_method(ctx, proto, "removeProcessingInstruction", g_id_remove_pi);
    idl_install_method(ctx, proto, "allowAttribute", g_id_allow_attribute);
    idl_install_method(ctx, proto, "removeAttribute", g_id_remove_attribute);
    idl_install_method(ctx, proto, "setComments", g_id_set_comments);
    idl_install_method(ctx, proto, "setDataAttributes", g_id_set_data_attributes);
    idl_install_method(ctx, proto, "removeUnsafe", g_id_remove_unsafe);
    JS_SetClassProto(ctx, g_class, JS_DupValue(ctx, proto));
    ctor = idl_step_constructor(ctx, "Sanitizer", g_id_ctor);
    CHECK(!JS_IsException(ctor), "the Sanitizer interface object could not be allocated");
    JS_SetConstructor(ctx, ctor, proto);
    JS_FreeValue(ctx, proto);
    global = JS_GetGlobalObject(ctx);
    idl_define_global_property_reference(ctx, global, "Sanitizer", ctor);
    JS_FreeValue(ctx, global);
}

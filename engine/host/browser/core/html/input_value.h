...
/* §4.10.5's TYPE CHANGE STEPS — "When an input element's type attribute changes state, the user agent must run
   ...
   three transfer branches and step 7 is a condition on — cannot be read off the element any more.
   ITS ONE CALL SITE is core/dom/element.c's element_attr_changed, beside media_element_attr_changed and
   html_option_attr_changed.

   NAMED RESIDUAL — the steps built here are HTML §4.10.5's 1, 2, 3, 6 and 7-9; its steps 4 and 5 are NOT
   built, and each needs a component this file does not own:

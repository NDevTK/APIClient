                                              ...
                                              JSValueConst *argv, int magic)
{
    ...
    if (strcmp(type, "text/html") != 0) {
        ...
        DFAIL("HTML §8.5.1 parseFromString reached its XML arm and this build has no XML parser (lexbor ships "
              ...
              "xml_markup.h is §2.7's [18] CDSect — the construct in which only [21] CDEnd is markup, so its "
              "content never reaches xml_ref.h — and "
              ...
              "/ [42] ETag / [44] EmptyElemTag and [43] content, §2.5's [15] Comment, §2.6's [16] PI, "
              "and §3.3.3 attribute-value normalization over [10] AttValue, which is written over "
              "xml_ref.h's result. Then route this arm to it, build §8.5.1 step 3's "

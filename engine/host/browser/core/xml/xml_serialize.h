 ...
 * containing no children, are serialized using the empty-element tag syntax (i.e., according to the XML
 * EmptyElemTag production)". Neither question exists in §13.3 at all, and answering them is a per-element
 * NAMESPACE PREFIX MAP (§3.2.1.1.2) copied down the tree plus a generated-prefix counter shared across the
 * whole serialization. So this is a second walk, not a flag on the first one.
 *
 * IT IS AN EMBEDDABLE ALGORITHM AND NOT A MEMBER, for the reason DOM §4.4's `clone a node` is one
 * (core/dom/node.h): §3.2.1 has TWO consumers in the standards and they are in different specifications.
 ...
 * and outerHTML getters. A member that owned the walk privately would be a second transcription of §3.2.1 the
 ...
 * per caller with that caller's own words in front of §3.2.1's steps — which is what keeps two consumers'
 ...
 * page says. §3.2.1.1 step 19 recurses over children, and a C recursion there is a stretch of the page's size
 * with nowhere for the scheduler to park — CLAUDE.md §scheduler's razor. One node per step, one attribute per
 * step, and the level stack is what the recursion became.
 *
 * WHERE §3.2.1's `throw an exception` LANDS. §3.2.1 step 5 wraps the whole algorithm: "If an exception occurs
 * during the execution of the algorithm, then catch that exception and throw an 'InvalidStateError'
 * DOMException." Every inner throw in §3.2.1.1 through §3.2.1.7 is therefore observable ONLY as that one
...
/* §3.2.1.1.2 The Namespace Prefix Map, AS ONE APPEND-ONLY ARRAY OF PAIRS, and the representation is the reason
 * "copy a namespace prefix map" costs nothing here.
 *
 * §3.2.1.1.2 defines the map as "namespaceURI values are the map's unique keys ... and ordered lists of
 * associated prefix values are the map's key values", and every operation it defines is stated over the
 * candidates LIST for one key: "retrieve a list from map where there exists a key in map that matches the value
 * of ns", "append prefix to the end of candidates list", "if the value of prefix occurs at least once in
 * candidates list". A flat array of (ns, prefix) pairs in INSERTION ORDER carries exactly that: the candidates
 * list for a key is the subsequence of pairs with that key, in the order they were added, and "create a new
 * list with prefix as the only item" and "append prefix to the end" are the SAME append.
 *
 * AND §3.2.1.1 step 6's copy is then a LENGTH. The map is only ever mutated by "add a prefix", which appends,
 ...
 *
 * IT IS ALSO §3.2.1.1's LOCAL PREFIXES MAP, which is prefix→namespaceURI rather than the other way round: the
...
typedef struct {
    lxb_dom_node_t *node;    /* the element whose §3.2.1.1 step 20 end tag this level owes; NULL at the root */
    lxb_dom_node_t *limit;   /* the container to restore — what `node` was itself a child of */
    uint32_t        qname;   /* §3.2.1.1's `qualified name`, as step 20 writes it */
    uint32_t        ctx_ns;  /* §3.2.1's context namespace to restore, so a SIBLING is not given ours */
    int             map_n;   /* §3.2.1.1 step 6: the map length the copy was made at */
} XmlSerLevel;

/* THE WHOLE OF §3.2.1's STATE. Every pointer here is js_malloc'd and declared to xml_serialize_visit_state, so
...
typedef struct {
    /* THE WALK */
    lxb_dom_node_t *cur;      /* the node §3.2.1's dispatch is standing on */
    lxb_dom_node_t *limit;    /* the container `cur` is a child of; NULL when `cur` is the root itself */
    lxb_dom_attr_t *attr;     /* the attribute §3.2.1.1.1's Main / §3.2.1.1.3's Loop is standing on */
    XmlSerLevel    *stack;
    int             sp, scap;

    /* THE OUTPUT — §3.2.1.1's `markup` and §3.2.1.1.3's `result`, accumulated once rather than per node. */
    char   *out;
    size_t  out_len, out_cap;

    /* THE STRING POOL. Every namespaceURI, prefix and qualified name this algorithm compares is interned here,
       so "is equal to" is an id comparison and the map is a pair of ints. Id 0 is §3.2.1's `null` and is never
       a string: the empty string is interned like any other, which is what lets §3.2.1.1.1's "replacing the
       value of null with the empty string" be a real distinction rather than a lost one. */
    char     *pool;
    size_t    pool_len, pool_cap;
    uint32_t *pool_ent;
    int       pool_n, pool_ecap;

    /* §3.2.1.1.2's map, §3.2.1.1's `local prefixes map`, and §3.2.1.1.3's `localname set`. */
    XmlSerPair *map;     int map_n, map_cap;
    XmlSerPair *local;   int local_n, local_cap;
    XmlSerPair *lnset;   int lnset_n, lnset_cap;

    /* THE FOUR ARGUMENTS §3.2.1 says every one of its algorithms is passed, plus the element in flight. */
    ...
    uint32_t elem_ns;        /* §3.2.1.1 step 10's `ns` */
    uint32_t inherited_ns;   /* §3.2.1.1 step 9's `inherited ns` — the context namespace of this element's children */
    uint32_t qname;          /* §3.2.1.1 step 3's `qualified name` */
    uint32_t local_default;  /* §3.2.1.1 step 8's `local default namespace`; 0 is its null */
    bool     ignore_nsdef;   /* §3.2.1.1 step 5's `ignore namespace definition attribute` */
    bool     skip_end_tag;   /* §3.2.1.1 step 4's `skip end tag` */
...
/* §3.2.1's STAGES, expanded into the caller's own block. `W` is the caller's leading text and `P` its prefix. */
#define XML_SERIALIZE_ALGO_STAGES(X, P, W) \
    X(P##_DISPATCH, W " → DOM Parsing and Serialization §3.2.1 XML Serialization: the XML serialization " \
                      "algorithm's dispatch on node's interface, and §3.2.1.1 XML serializing an Element node " \
                      "steps 1-7 (the localName check and the per-element copy of the namespace prefix map)") \
    X(P##_RECORD,   W " → DOM Parsing and Serialization §3.2.1.1.1 Recording the namespace: Main, one " \
                      "attribute of element's attributes per step") \
    X(P##_NAME,     W " → DOM Parsing and Serialization §3.2.1.1 XML serializing an Element node steps 9-12 " \
                      "(the element's qualified name, and the one namespace declaration its start tag carries)") \
    X(P##_ATTRS,    W " → DOM Parsing and Serialization §3.2.1.1.3 Serializing an Element's attributes: Loop, " \
                      "one attribute of element's attributes per step") \
    X(P##_OPEN,     W " → DOM Parsing and Serialization §3.2.1.1 XML serializing an Element node steps 14-19 " \
    ...
    X(P##_LEAF,     W " → DOM Parsing and Serialization §3.2.1.3 XML serializing a Comment node, §3.2.1.4 XML " \
                      "serializing a Text node, §3.2.1.6 XML serializing a DocumentType node and §3.2.1.7 XML " \
                      "serializing a ProcessingInstruction node (this node's own markup)") \
    X(P##_NEXT,     W " → DOM Parsing and Serialization §3.2.1.2 XML serializing a Document node, §3.2.1.5 XML " \
                      "serializing a DocumentFragment node and §3.2.1.1 XML serializing an Element node step " \
                      "19 (the step to the next child in tree order)") \
...
/* §3.2.1's result, as the DOMString the member returns. Valid only at the caller's `after` stage. */

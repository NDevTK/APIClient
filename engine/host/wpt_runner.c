 ...
 * lists `METHOD<TAB>DESTINATION<TAB>INITIATOR<TAB>PROVENANCE<TAB>PINNED<TAB>CREDENTIALS<TAB>URL` and engine_provide delivers against the
...
static int wpt_issue_pending(void)
{
    ...
    for (p = list; *p; ) {
        ...
        *end = 0;
        ...
        engine_pending_split(p, &method, &destination, &initiator, &provenance, &pinned, &credentials, &url);
...
/* THIS DOCUMENT'S PROGRAM SEQUENCE — what a session is opened over (engine.h). Each entry is its own program
   ...
   `body` is taken (owned here from this point), `name` is copied, and `type` is HTML §4.12.1.1's script type —
                                     ...
                                     const char *ancestor_origins, const char *creation_sandbox_flags)
{
    ...
    /* §7.4.5's COMPUTED TYPE for the document about to be built, and whether there is one at all. NO RESPONSE
       is not a default and not a guess: §7.4's initial about:blank — and an address whose fetch failed, for
       which a browser still shows a document — has no response to compute a type from and IS an HTML document
       by that section, so the parse below runs §7.4 rather than §7.4.5's dispatch for it. It lives across the
       whole function because the type and the bytes travel TOGETHER to the loader; a record computed and freed
    ...
    /* HTML §7.4.5's FIRST STEP — "Let type be the computed type of navigationParams's response" — asked HERE,
       ...
       component answers it for core/frame/navigable.c's child_document, and this runner did not ask it at all:
       every top-level test document went to the HTML parser whatever the server served.
    ...
    /* SHARED, for main.c's reason: this is the runner's ACTIVE document and every flow reads its tree.
       THE TWO CALLS ARE §7.4.5's LOAD-A-DOCUMENT AND §7.4's INITIAL about:blank — see child_document, which
...
int main(int argc, char **argv)
{
    ...
    /* HTML §8.1.4.6 Runtime script errors AND DOM §2.2's Event with the thirteen subclasses
       core/events/event.c declares beside it, which used to be the two lines here — and THIS host is the one
       that shows why the column and not the line: all three had both, and this one ran them BEFORE
       realm_intrinsics_free while the other two ran them after. Nothing was missing and the order was still
       three different answers. Both are ROWS on core/platform.h's release column now, run by the
       platform_agent_free above, and out here neither component's state could be declared to
       core/agent_state.h at all — a row with agent state and no release is what platform_check_agent_state

                  ...
                  const char *replies, const char *csp)
{
    ...
    install_dom_interface_ctors(ctx, g, g_el_proto);   /* EventTarget..SVGElement base ctors + customElements registry (custom_elements.c) */
    ...
    JS_SetPropertyStr(ctx, g, "showOpenFilePicker", JS_NewCFunction(ctx, js_fsa_open_picker, "showOpenFilePicker", 1));   /* File System Access (browser/fsa.c): a picked file's content is an attacker @S source */

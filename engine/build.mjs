...
const sources = ["quickjs.c", "libregexp.c", "libunicode.c", "dtoa.c", "quickjs-libc.c"]
  ...
  .concat([join(HOST, "main.c"), join(HOST, "prelude.c"),
    SOLVER("solve_html.c"), SOLVER("solve_js.c"), SOLVER("dom_cow.c"), SOLVER("concolic.c"), SOLVER("concolic_array.c"), SOLVER("reply.c"), SOLVER("endpoint.c"), SOLVER("attr_shadow.c"), SOLVER("constraints.c"), SOLVER("boot_scripts.c"), SOLVER("parked.c"), SOLVER("why.c"), SOLVER("envelope.c"), SOLVER("solve.c"), SOLVER("source.c"), SOLVER("async_flow.c"), SOLVER("boot_flow.c"), SOLVER("scheduler.c"),

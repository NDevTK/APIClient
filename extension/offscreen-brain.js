// offscreen-brain.js — the API-learning brain + the v2 host bridge. Runs in the
// OFFSCREEN document (loaded by ast-worker.html, which provides the lib/* globals
// via <script> tags). It owns globalStore + discovery + AST merge + the request log
// + ALL the popup message handlers, and persists to IndexedDB — a stable home,
// unlike the evicted service worker. The thin service worker (background.js)
// forwards browser events here and performs privileged chrome.tabs.* calls on this
// brain's behalf (see _swRpc / __rpc).
//
// The v2 host bridge (formerly a separate ast-worker.js, now DELETED + merged at the
// bottom of this file) dynamic-import()s the engine WASM (lib/qjs/qjs.mjs), drives the
// qjs_step protocol, safe-fetches replies/chunks, persists the cross-session frontier,
// and JSON.parses the engine's ONE @RESULT — reached via self.astDispatch. It is the
// ONLY trusted zone (SECURITY.md): all network/IDB/chrome/popup live here, never in
// the untrusted WASM. No analysis LOGIC here — the C engine owns that.
...
async function getAnalyzerFingerprint() {
  ...
  _analyzerFingerprintP = (async () => {
    var files = ["offscreen-brain.js", "lib/qjs/qjs.mjs"];   // brain+bridge + engine module (the bridge merged into offscreen-brain.js; a wasm rebuild that changes behavior bumps qjs.mjs's embedded fingerprint)
    var hashes = [];
...
/* ============================================================================
   v2 HOST BRIDGE (merged from ast-worker.js — the file was DELETED). The irreducible trusted relay:
   loads the engine WASM (dynamic import), drives the qjs_step protocol, safe-fetches replies/chunks,
   persists the cross-session frontier to IndexedDB, and JSON.parses the engine's ONE @RESULT. No
   analysis LOGIC lives here (the engine owns it) — only the SECURITY.md-mandated platform edges.
   ============================================================================ */

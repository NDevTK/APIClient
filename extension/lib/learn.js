...
function learnFromAstCallSite(docData, interfaceName, callSite, scriptUrl) {
  ...
  /* THE RAW BODY IS CARRIED AND NOT DECODED, AND THE ARM THAT DECODED IT IS DELETED RATHER THAN NARROWED.
     It stripped a gRPC-Web frame by testing the content-type against a pattern and then reconstructed field
     numbers and wire types out of the bytes. Both halves are wrong and each is wrong on its own rule.
     §RUN, DON'T MATCH: values are what the engine COMPUTES. A field number recovered from a byte stream is
     INFERRED — the page's own serializer already ran, with the real names and the real values in its hands,
     and reading the output back is guessing at an answer the run had. It is also exactly the shape
     endpoint.c's own contract refused on purpose — "records no fields rather than a guess at some" — which is
     a decision somebody reasoned through, and overriding it with a decoder is the deliberate-row defect.
     AND IT WAS PROTOCOL-SPECIFIC, which nothing in this engine may be. A `grpc-web` pattern over a MIME
     string is bundler-recognition one layer down: the next transport is Connect, or grpc-web-text, or a
     framing somebody invents next year, and each one is another branch nobody can be complete about.
     WHAT THE BYTES ARE STILL FOR: they are an OBSERVATION, not an inference — this request sent exactly these
     bytes under exactly this content-type — so a reviewer can replay it. They are not an answer to "what
     values can be sent", and must not be read as one.
     NAMED RESIDUAL. WHAT IS NOT COVERED: the fields and example values of a body this engine has no reader
     for. WHAT THE NEXT DIFF BUILDS: the per-flow character/segment provenance §@S already runs through a
     real filter, carried through the page's OWN serializer, so the body's bytes say which source produced
     which span — which needs no schema, no transport branch and no decoder, because the program did the
     encoding. HOW ITS ABSENCE WOULD SHOW: a request record carrying bytes a reviewer can replay verbatim and
     no field a reviewer can vary. */

...
const STORE_RECORD_KINDS = Object.freeze({
  ...
  endpoints: Object.freeze({
    /* 8, RAISED WITH lib/persistence.js's `_STORE_SHAPE` IN THE SAME DIFF — the pair that file's own comment
       forbids splitting. It was 6 for `bodySent` and `bodyShape`, 7 for `bodyExample` (the third thing a
       request body can be), and is 8 for `pathParamDomains` — what the run's own gates PROVED about each
       templated hole, which this record carried the VALUES of and none of, so lib/send.js stated four
       absences it had never observed. `checkEndpointRecord` asserts it exactly as it asserts the body three;
       a store written before each of them carries none of the keys that diff added. At `1` this kind was asserted
       ...
       cannot be derived. */
    statedFrom: 8, delegate: "endpoint",

...
(function () {
  ...
  // ─── Exploit probe mode ─────────────────────────────────────────────────────
  //
  // When the page's URL carries a `__apisec=MARKER` token in the search
  // query or hash, install DOM/code-exec/network sink wrappers that
  // record every call site whose value contains the marker. The probe is
  // opt-in per-navigation: pages without the marker pay no overhead.
  //
  // The marker is a random 8-char token generated by the background
  // worker. Every hit is dispatched to the content-script relay via a
  // dedicated CustomEvent so the service worker can correlate hits back
  // to the probe session.
  try {
    const u = new URL(location.href);
    const m = (u.hash.match(/[#&]__apisec=([A-Za-z0-9_]+)/) ||
               u.search.match(/[?&]__apisec=([A-Za-z0-9_]+)/) || [])[1];
    if (m) installProbe(m);
  } catch (_) { /* probe installation is best-effort; never break the page */ }

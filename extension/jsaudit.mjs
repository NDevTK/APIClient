...
const LEDGER = [
  ...
  { f: "offscreen-brain.js", zone: "MIXED", step: 6, dest: "engine/host/solver/moat.c + bridge.js",
    split: "globalStore, the request log, _handleFormSubmit, buildExportRequest and buildPocFromShape are LOGIC; buildPocFromShape is also a SECOND PoC constructor beside the engine's (§LIVE VERIFY: never a re-derived PoC — the delivery vector belongs beside the attacker-source constraints in C). The chrome.runtime.onMessage router + the sender.tab.url trust gate are BRIDGE:3 and fold into bridge.js." },
